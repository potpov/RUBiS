...
<html>
  <body>
    ...
    $regionResult = mysqli_query($link, "SELECT * FROM regions WHERE name=\"$region\"");
	if (!$regionResult)
	{
		error_log("[".__FILE__."] Query 'SELECT * FROM regions WHERE name=\"$region\"' failed: " . mysqli_error($link));
		die("ERROR: Region query failed for region '$region': " . mysqli_error($link));
	}
    if (mysqli_num_rows($regionResult) == 0)
    {
      ...
      mysqli_free_result($regionResult);
      commit($link);
      exit();
    }
    else
    ...
    $nicknameResult = mysqli_query($link, "SELECT * FROM users WHERE nickname=\"$nickname\"");
	if (!$nicknameResult)
	{
		error_log("[".__FILE__."] Query 'SELECT * FROM users WHERE nickname=\"$nickname\"' failed: " . mysqli_error($link));
		die("ERROR: Nickname query failed: " . mysqli_error($link));
	}
    if (mysqli_num_rows($nicknameResult) > 0)
    {
      ...
      mysqli_free_result($nicknameResult);
      exit();
    }
    mysqli_free_result($nicknameResult);
    ...
    $result = mysqli_query($link, "INSERT INTO users VALUES (NULL, \"$firstname\", \"$lastname\", \"$nickname\", \"$password\", \"$email\", 0, 0, '$now', $regionId)");
	if (!$result)
	{
		error_log("[".__FILE__."] Failed to insert new user in database INSERT INTO users VALUES (NULL, \"$firstname\", \"$lastname\", \"$nickname\", \"$password\", \"$email\", 0, 0, '$now', $regionId)': " . mysqli_error($link));
		die("ERROR: Failed to insert new user '$nickname' in database: " . mysqli_error($link));
	}
	...
	{
		error_log("[".__FILE__."] Query 'SELECT * FROM users WHERE nickname=\"$nickname\"' failed: " . mysqli_error($link));
		die("ERROR: Query user failed: " . mysqli_error($link));
	}
