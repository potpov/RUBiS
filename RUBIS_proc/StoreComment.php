...
//	}
    // Update user rating
    $toRes = mysqli_query($link,"SELECT rating FROM users WHERE id=\"$to\"");
	if (!$toRes)
	{
		error_log("[".__FILE__."] Query 'SELECT rating FROM users WHERE id=\"$to\"' failed: " . mysqli_error($link));
		die("ERROR: User query failed for user '$to': " . mysqli_error($link));
	}
    if (mysqli_num_rows($toRes) == 0)
    {
      printError($scriptName, $startTime, "StoreComment", "<h3>Sorry, but this user does not exist.</h3><br>");
      exit();
    }
    $userRow = mysqli_fetch_array($toRes);
    $rating = $rating . $userRow["rating"];
    $result = mysqli_query($link,"UPDATE users SET rating=$rating WHERE id=$to");
	if (!$result)
	{
		error_log("[".__FILE__."] Unable to update user's rating 'UPDATE users SET rating=$rating WHERE id=$to': " . mysqli_error($link));
		die("ERROR: Unable to update user's rating for user '$to': " . mysqli_error($link));
	}

    // Add bid to database
    $now = date("Y:m:d H:i:s");
    $result = mysqli_query($link,"INSERT INTO comments VALUES (NULL, $from, $to, $itemId, $rating, '$now', \"$comment\")");
	if (!$result)
	{
		error_log("[".__FILE__."] Failed to insert new comment in database 'INSERT INTO comments VALUES (NULL, $from, $to, $itemId, $rating, '$now', \"$comment\")': " . mysqli_error($link));
		die("ERROR: Failed to insert new comment in database: " . mysqli_error($link));
	}
