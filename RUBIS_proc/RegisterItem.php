...
<html>
  <body>
    ...
    $result = mysqli_query($link, "INSERT INTO items VALUES (NULL, \"$name\", \"$description\", $initialPrice, $qty, $reservePrice, $buyNow, 0, 0, '$start', '$end', $userId, $categoryId)");
	if (!$result)
	{
		error_log("[".__FILE__."] Failed to insert new item in database. mysqli reports '".mysqli_error($link)."' while querying 'INSERT INTO items VALUES (NULL, \"$name\", \"$description\", $initialPrice, $qty, $reservePrice, $buyNow, '$start', '$end', $userId, $categoryId)': ");
		die("ERROR: Failed to insert new item in database. mysqli reports '".mysqli_error($link)."' while querying 'INSERT INTO items VALUES (NULL, \"$name\", \"$description\", $initialPrice, $qty, $reservePrice, $buyNow, '$start', '$end', $userId, $categoryId)'");
	}
    commit($link);
