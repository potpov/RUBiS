...
<html>
  <body>
	...
	if (!$result)
	{
		...
		die("ERROR: Item query failed for item '$itemId': " . mysqli_error($link));
	}
    ...
    $toRes = mysqli_query($link, "SELECT * FROM users WHERE id=\"$to\"");
	if (!$toRes)
	{
		error_log("[".__FILE__."] Query 'SELECT * FROM users WHERE id=\"$to\"' failed: " . mysqli_error($link));
