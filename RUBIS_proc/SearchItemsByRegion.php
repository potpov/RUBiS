...
<html>
  <body>
    ...
    $result = mysqli_query($link, "SELECT items.id,items.name,items.initial_price,items.max_bid,items.nb_of_bids,items.end_date FROM items,users WHERE items.category=$categoryId AND items.seller=users.id AND users.region=$regionId AND end_date>=NOW() LIMIT ".$page*$nbOfItems.",$nbOfItems");
	if (!$result)
	{
		error_log("[".__FILE__."] Query 'SELECT items.id,items.name,items.initial_price,items.max_bid,items.nb_of_bids,items.end_date FROM items,users WHERE items.category=$categoryId AND items.seller=users.id AND users.region=$regionId AND end_date>=NOW() LIMIT ".$page*$nbOfItems."' failed: " . mysqli_error($link));
		die("ERROR: Query failed for category '$categoryId', region '$regionId', page '$page' and nbOfItems '$nbOfItems': " . mysqli_error($link));
	}
