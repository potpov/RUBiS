...
<html>
  <body>
   ...
   printHTMLheader("RUBiS: About me");
    ...
    $bidsResult = mysqli_query($link, "SELECT item_id, bids.max_bid FROM bids, items WHERE bids.user_id=$userId AND bids.item_id=items.id AND items.end_date>=NOW() GROUP BY item_id, bids.max_bid");
	if (!$bidsResult)
	{
