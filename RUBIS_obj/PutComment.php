...
<html>
  ...
  try{
      switch($_SERVER['REQUEST_METHOD']) {
          case "POST":
              //do post
              $userFrom = User_class::LoadUserByCredential($_POST['nickname'], $_POST['password']);
              $item =  Items_class::LoadItemByID($_POST['itemId']);
              $userTo = User_class::LoadUserByID($_POST['to']);
              break;
          case "GET":
              //do get;
              $userFrom = User_class::LoadUserByCredential($_GET['nickname'], $_GET['password']);
      ...
      $log->PutCommentPage($userTo, $userFrom, $item);
