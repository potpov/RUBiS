...
<html>
  ...
  try{
      switch($_SERVER['REQUEST_METHOD']) {
          case "POST":
              //do post
              $buyer = User_class::LoadUserByCredential($_POST['nickname'], $_POST['password']);
              $item =  Items_class::LoadItemByID($_POST['itemId']);
              break;
          case "GET":
              //do get;
              $buyer = User_class::LoadUserByCredential($_GET['nickname'], $_GET['password']);
      ...
      $seller = User_class::LoadUserByID($item->getSeller());
      $log->BuyNowPage($buyer, $seller, $item);
  } catch (Exception $e) {
